...
from pokerWinner.handeval import Evaluate #Module I made for calculating hand ranks
...
class PokerTests(unittest.TestCase):
	...
	def testHighCard(self):
		...
		self.assertEquals(self.e.tieBreaker('5H TC QS 9D 6H'.split(), '5S AC QH 4C TD'.split(), 1), '5S AC QH 4C TD'.split())

	def testHighestDup(self):
		#This tests which hand has the highest 3 of a kind
		self.assertEquals(self.e.tieBreaker('5H TC TS 9D TH'.split(), 'QS AC QH QC TD'.split(), 4), 'QS AC QH QC TD'.split())
	...
	def testHighestPair(self):
		...
		self.assertEquals(self.e.tieBreaker('AH QC QS AD 9H'.split(), 'AS AC QH QC TD'.split(), 2), 'AS AC QH QC TD'.split())
